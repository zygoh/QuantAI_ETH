...
class MLService:
    ...
    def __init__(self):
        ...
        self.lgb_params_by_timeframe = {
            '15m': {
                'num_leaves': 127,       # 数据翻倍(27,648条)，增加复杂度
                'min_child_samples': 30,  # 每叶子约217条样本
                'max_depth': 10          # 稍深一点
            },
            '2h': {
    ...
    def _create_labels(self, df: pd.DataFrame, timeframe: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            # 中频交易策略：目标HOLD<30%，LONG/SHORT各35%左右
            threshold_config = {
                '15m': {
                    'up': 0.001,      # ±0.1% ← 中频交易：灵敏，HOLD 24-32%
                    'down': -0.001    # $4000 × 0.1% = $4波动
                },
                '2h': {

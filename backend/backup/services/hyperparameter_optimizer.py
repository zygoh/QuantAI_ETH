                ...
                if self.model_type == "lightgbm":
                    ...
                    if use_amp:
                        ...
                        if num_params > 10_000_000:  # >10M参数：大模型
                            ...
                            logger.debug(f"   检测到大模型({num_params/1e6:.1f}M参数)，使用init_scale=2^12")
                        elif num_params > 1_000_000:  # 1M-10M参数：中等模型
                            init_scale = 2.**14  # 16384
                            logger.debug(f"   检测到中等模型({num_params/1e6:.1f}M参数)，使用init_scale=2^14")
                        else:  # <1M参数：小模型
                            init_scale = 2.**16  # 65536（默认值）
                            logger.debug(f"   检测到小模型({num_params/1e6:.1f}M参数)，使用init_scale=2^16")
                        
                        # ✅ 修复：使用新的torch.amp.GradScaler API（PyTorch 2.0+）
                        # ✅ 关键修复：更保守的缩放器配置，防止scale无限增长
                        scaler = torch.amp.GradScaler(
                            'cuda',
                            init_scale=init_scale,  # 动态调整的初始缩放
                            growth_factor=1.2,      # ✅ 从1.5降低到1.2，更温和的增长
                            backoff_factor=0.5,     # 回退因子（检测到溢出时）
                            growth_interval=2000,   # ✅ 从1000增加到2000，更谨慎的增长
                            enabled=True
                        )
                        torch.backends.cuda.matmul.allow_tf32 = True
                        torch.backends.cudnn.allow_tf32 = True
                        logger.debug(f"   混合精度训练: 启用（动态缩放策略，初始缩放: {init_scale}）")
                    else:
                        scaler = None
                        ...
                        for i, (batch_X, batch_y) in enumerate(train_loader):
                            ...
                            if (i + 1) % accumulation_steps == 0 or (i + 1) == len(train_loader):
                                ...
                                if use_amp:
                                    """
                                    混合精度训练梯度更新流程（生产级标准）：
                                    1. scaler.step(optimizer) - 执行优化器更新（内部会检查溢出）
                                    2. scaler.update() - 更新缩放器状态（如果检测到溢出会自动backoff）
                                    3. 检查缩放器scale，如果仍然过大则重置（避免scale无限增长）
                                    
                                    关键原则：
                                    - 必须在step之后调用update，否则会报错"No inf checks were recorded"
                                    - 重置scaler必须在update之后，避免影响当前batch的梯度更新
                                    - 使用保守的阈值（1e5）和更小的初始scale，防止数值不稳定
                                    """
                                    # ✅ 生产级标准：先执行step和update，确保当前batch梯度更新完成
                                    scaler.step(optimizer)
                                    scaler.update()  # ✅ 必须在step之后调用，会自动检测溢出并backoff
                                    
                                    # ✅ 生产级标准：在update之后检查scale，如果仍然过大则重置
                                    # 阈值说明：
                                    # - 1e5 (100,000): 警告阈值，记录日志但不立即重置
                                    # - 1e6 (1,000,000): 严重阈值，强制重置缩放器
                                    current_scale = scaler.get_scale()
                                    
                                    if current_scale > 1e6:  # 严重阈值：强制重置
                                        logger.warning(
                                            f"⚠️ Trial {trial.number} Fold {fold_idx+1} Epoch {epoch+1} Batch {i+1}: "
                                            f"缩放器scale严重过大 ({current_scale:.2f})，强制重置"
                                        )
                                        
                                        # ✅ 生产级标准：根据模型规模动态调整初始scale
                                        num_params = sum(p.numel() for p in model.parameters())
                                        if num_params > 10_000_000:  # >10M参数：大模型
                                            new_init_scale = 2.**10  # 1024
                                        elif num_params > 1_000_000:  # 1M-10M参数：中等模型
                                            new_init_scale = 2.**12  # 4096
                                        else:  # <1M参数：小模型
                                            new_init_scale = 2.**14  # 16384
                                        
                                        # ✅ 生产级标准：重新创建缩放器，使用更保守的配置
                                        scaler = torch.amp.GradScaler(
                                            'cuda',
                                            init_scale=new_init_scale,
                                            growth_factor=1.2,      # 温和增长（从1.5降低）

...
def add_sentiment_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if 'rsi_14' in df.columns:
            ...
            new_features['rsi_momentum'] = rsi - rsi.shift(5)
            new_features['rsi_volatility'] = rsi.rolling(10).std()
        ...
        if 'volume' in df.columns:
            ...
            new_features['volume_surge'] = (df['volume'] > volume_ma * 2).astype(int)
            new_features['volume_dry'] = (df['volume'] < volume_ma * 0.5).astype(int)
        ...
        if 'macd_histogram' in df.columns:
            macd_norm = df['macd_histogram'] / (df['close'] * 0.01 + 1e-10)
            sentiment_score += np.clip(macd_norm, -1, 1)
        
        sentiment_score += price_change.rolling(10).mean() * 100
        ...
        new_features['extreme_move_decay'] = new_features['extreme_move'].rolling(5).sum()

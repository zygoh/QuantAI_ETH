...
class EnsembleMLService(MLService):
    ...
    def _predict_scalar(self, model: Any, X: np.ndarray, model_name: str = "model") -> int:
        """
        统一预测接口，确保返回标量值
        
        Args:
            model: 训练好的模型（LightGBM/XGBoost/CatBoost）
            X: 特征数据（单样本或批量，应为2D数组）
        ...
        try:
            ...
            if isinstance(pred, (np.ndarray, list)):
                if len(pred) == 0:
                    raise ValueError(f"{model_name}预测返回空数组")
                # 取第一个元素
            ...
            # 验证预测值在有效范围内（0=SHORT, 1=HOLD, 2=LONG）
            if scalar_pred not in [0, 1, 2]:
                logger.warning(f"⚠️ {model_name}预测值超出范围: {scalar_pred}，将限制到[0,2]")
                scalar_pred = max(0, min(2, scalar_pred))
            
            logger.debug(f"✅ {model_name}预测: {scalar_pred} (类型: {type(scalar_pred).__name__})")
            return scalar_pred
            
        except Exception as e:
            logger.error(f"❌ {model_name}标量预测失败: {e}")
            logger.error(f"   输入形状: {X.shape}, 预测结果类型: {type(pred).__name__}")
            raise ValueError(f"{model_name}预测失败: {e}")
    ...
    def _generate_enhanced_meta_features(
        ...
        try:
            ...
            # 基础模型预测结果（单样本）
            # 🔑 关键修复：使用统一的_predict_scalar方法确保所有预测值都是标量（int）
            lgb_pred = self._predict_scalar(models['lgb'], X_pred, model_name="LightGBM")
            
            # xgb_pred 已经从_predict_xgboost获取，但需要确保是标量
            if isinstance(xgb_pred, (np.ndarray, list)):
                xgb_pred = int(xgb_pred[0] if len(xgb_pred) > 0 else xgb_pred)
            else:
                xgb_pred = int(xgb_pred)
            
            cat_pred = self._predict_scalar(models['cat'], X_pred, model_name="CatBoost")
            ...
            try:
                ...
                logger.debug(f"✅ 预测数组创建成功: {pred_array}, 分歧度: {pred_disagreement:.4f}")
            except Exception as e:
                logger.error(f"❌ 预测数组创建失败: {e}")
                logger.error(f"   lgb_pred类型: {type(lgb_pred)}, 值: {lgb_pred}")
                logger.error(f"   xgb_pred类型: {type(xgb_pred)}, 值: {xgb_pred}")
                logger.error(f"   cat_pred类型: {type(cat_pred)}, 值: {cat_pred}")
                raise
            ...
            # 数组维度错误或特征数量不匹配
            logger.error(f"❌ 增强元特征生成失败（ValueError）: {e}")
            logger.error(f"   X_pred形状: {X_pred.shape}")
            logger.error(f"   lgb_pred类型: {type(lgb_pred) if 'lgb_pred' in locals() else 'N/A'}")
            logger.error(f"   xgb_pred类型: {type(xgb_pred) if 'xgb_pred' in locals() else 'N/A'}")
            logger.error(f"   cat_pred类型: {type(cat_pred) if 'cat_pred' in locals() else 'N/A'}")
            
            # 返回默认特征（与训练时保持一致）
            if 'inf' in models or (hasattr(models.get('meta'), 'n_features_') and models['meta'].n_features_ == 45):
                default_features = np.zeros(45)  # 45个特征（基础25+新增20）
                logger.warning(f"⚠️ 使用默认特征（45个，含Informer-2占位符）")
            else:
                default_features = np.zeros(40)  # 40个特征（基础20+新增20）
                logger.warning(f"⚠️ 使用默认特征（40个，不含Informer-2）")
            return default_features.reshape(1, -1)
            
        except AssertionError as e:
            # 特征数量验证失败
            logger.error(f"❌ 元特征数量验证失败: {e}")
            logger.error(f"   期望特征数: {expected_features if 'expected_features' in locals() else 'N/A'}")
            logger.error(f"   实际特征数: {len(all_features) if 'all_features' in locals() else 'N/A'}")
            
            # 返回默认特征
            if 'inf' in models or (hasattr(models.get('meta'), 'n_features_') and models['meta'].n_features_ == 45):
                default_features = np.zeros(45)
                logger.warning(f"⚠️ 使用默认特征（45个）")
            else:
                default_features = np.zeros(40)
                logger.warning(f"⚠️ 使用默认特征（40个）")
            return default_features.reshape(1, -1)
            
        except Exception as e:
            # 其他未预期的错误
            logger.error(f"❌ 增强元特征生成失败（未知错误）: {e}", exc_info=True)

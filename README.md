...
**QuantAI-ETH** 是一个**专业量化工程师设计的生产级以太坊合约量化交易系统**，使用**Stacking集成学习**进行超高胜率中频交易。

### 核心特性

- 🏆 **超高胜率设计** - 胜率目标≥60%，盈亏比≥1.8:1（专业量化工程师标准）
- 🤖 **Stacking集成** - LightGBM + XGBoost + CatBoost三模型融合
- 📊 **顶尖准确率** - 81%平均准确率（15m:60%, 2h:90%, 4h:94%）
- ⏰ **多时间框架** - 15m/2h/4h独立训练与智能加权合成
- 🎯 **信号质量优先** - 5维度增强过滤，宁缺毋滥
- 🛡️ **完善风险控制** - 动态ATR止损，回撤保护，预热机制
- 📊 **实时数据流** - WebSocket实时推送 + Binance API
- 🔧 **工业级代码** - 智能特征选择，样本加权，HOLD偏好解决

---

## 📊 性能指标

### 当前状态（v2.0 - Stacking Ensemble）

| 指标 | 数值 | 状态 | 目标 |
|------|------|------|------|
| **平均准确率** | **81.33%** | ✅ 超额62.66% | ≥50% |
| **15m准确率** | **60.46%** | ✅ 超额10.46% | ≥50% |
| **2h准确率** | **90.13%** | ✅ 顶尖水平 | ≥50% |
| **4h准确率** | **94.39%** | ✅ 接近完美 | ≥50% |
| **特征数量** | 218个 → 150个 | ✅ 智能选择 | - |
| **模型架构** | Stacking集成 | ✅ | LGB+XGB+CAT |
| **HOLD预测** | 40-60% | ✅ 已解决 | 35-50% |
| **预期胜率** | ≥60% | 目标 | 超高胜率 |
| **预期盈亏比** | ≥1.8:1 | 目标 | 风险回报 |
...
- Python 3.12+
- FastAPI（异步Web框架）
- **LightGBM + XGBoost + CatBoost**（Stacking集成学习）
...
├── app/
│   ├── services/          # 核心业务
│   │   ├── ensemble_ml_service.py  # ⭐ Stacking集成模型（LGB+XGB+CAT）
│   │   ├── signal_generator.py     # ⭐ 信号生成（加权合成+动态降权）
│   │   ├── feature_engineering.py  # ⭐ 特征工程（218→150个特征）
│   │   ├── trading_engine.py       # 交易执行（虚拟/实盘）
│   │   ├── position_manager.py     # 仓位管理
│   │   ├── risk_service.py         # 风险管理
│   │   ├── scheduler.py            # 定时任务（训练后立即预测）
│   │   └── data_service.py         # WebSocket数据流
│   ├── api/               # REST API
│   └── core/              # 配置/数据库
└── models/                # Stacking模型（每个时间框架4个文件）
```
